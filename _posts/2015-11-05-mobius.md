...
$$ \begin{equation*} F(x) = \sum\_{d \mid x} f(d) \Leftrightarrow f(x)=\sum\_{d \mid x}\mu(d) F(\frac{x}{d}) \end{equation*} $$
$$ \begin{equation*} F(x) = \sum\_{x \mid d} f(d) \Leftrightarrow f(x)=\sum\_{x \mid d}\mu(\frac{d}{x}) F(d) \end{equation*} $$
...
我们令
$$ \begin{equation*} f(x) = \sum\_{i=1}^{n} \sum\_{j=1}^{m} \sum\_{x  \mid  gcd(i, j)} 1 \end{equation*} $$
...
$$ \begin{align*} 
f(x) &= \sum\_{x \mid d} \mu(\frac{d}{x}) F(d) \\\\
     &= \sum\_{x \mid d} \mu(\frac{d}{x}) \lfloor{\frac{n}{d}} \rfloor \lfloor{\frac{m}{d}} \rfloor \\\\
\end{align*} $$
...
ans &= \sum\_{p} f(p) & & \text{where p is a prime}. \\\\
    &= \sum\_{p} \sum\_{p \mid T}\mu(\frac{T}{p}) \lfloor{\frac{n}{T}} \rfloor \lfloor{\frac{m}{T}} \rfloor \\\\
    &= \sum\_{T=1}^{min(n, m)} \lfloor{\frac{n}{T}} \rfloor \lfloor{\frac{m}{T}} \rfloor  \sum\_{p \mid T}\mu(\frac{T}{p})  \\\\
\end{align*} $$

我们令 \\(g(T)=\sum\_{p \mid T} \mu(\frac{T}{p})\\)，那么如果能预处理出 \\(g(T)\\)的前缀和，
我们就能和上例一样分块搞了。
